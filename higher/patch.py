...
class _MonkeyPatchBasePeft(_abc.ABC):
    @_abc.abstractmethod
    def __init__(self) -> None:
        self._param_mapping: _typing.List[int] = []
        self._being_modified_internally: bool = True
...
def _make_functional(
    ...
    if not "peft" in str(_ModuleType):
        ...
        class MonkeyPatched(_ModuleType, _MonkeyPatchBasePeft):  # type: ignore
            _wrapped_name = type(module).__name__

            def __init__(self, original_params, root=None) -> None:
                _torch.nn.Module.__init__(self)
                _MonkeyPatchBasePeft.__init__(self)
                ...
                self._modules: _typing.Dict[str, _MonkeyPatchBasePeft] = _OrderedDict()

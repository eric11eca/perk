...
from . import ademamix as _ademamix
...
def linear_hl_warmup_scheduler(step, beta_end, beta_start=0, warmup=1):
    ...
    def f(beta, eps=1e-8):
        return _math.log(0.5)/_math.log(beta+eps)-1

    def f_inv(t):
        return _math.pow(0.5, 1/(t+1))
            ...
            for i, g in enumerate(all_grads):
                if g is not None:
                    if self._accumulated_grads[i] is None:
                        ...
                        self._accumulated_grads[i] = self._accumulated_grads[i] + g
...
class DifferentiableAdEMAMix(DifferentiableOptimizer):
    r"""A differentiable version of the AdEMAMix optimizer.
    ...
    def _update(self, grouped_grads: _GroupedGradsType, **kwargs) -> None:
        ...
        for group_idx, (group, grads) in enumerate(zipped):
            ...
            for p_idx, (p, grad) in enumerate(zip(group['params'], grads)):
                if grad is None:
                    continue
                if grad.is_sparse:
                ...
                if len(state) == 0:
                    ...
                    if beta1 != 0.0: # save memory in case beta1 is 0.0
                        state['exp_avg_fast'] = _torch.zeros_like(p, memory_format=_torch.preserve_format)
                    else:
                        state['exp_avg_fast'] = None
                    state['exp_avg_slow'] = _torch.zeros_like(p, memory_format=_torch.preserve_format)
                    # Exponential moving average of squared gradient values
                    state['exp_avg_sq'] = _torch.zeros_like(p, memory_format=_torch.preserve_format)
                ...
                denom = (exp_avg_sq.sqrt() / _math.sqrt(bias_correction2)).add_(eps)
...
_opt_mapping: _OptMappingType = {
    _ademamix.AdEMAMix: DifferentiableAdEMAMix,
